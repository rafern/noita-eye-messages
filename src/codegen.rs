...
fn sub32_messages(messages: &[&[u8]]) -> Vec<Vec<u8>> {
...
fn gen_message_structs() {
    for data in &sub32_messages(BASE10_PLUS_32) {
        let mut first = true;
        print!("Message {{\n    name: \"\",\n    data_len: {},\n    data: [", data.len());
        for c in data {
            if first {
        ...
        let pad_amount = MSG_LEN_MAX - data.len();
        first = true;
        if pad_amount > 0 {
